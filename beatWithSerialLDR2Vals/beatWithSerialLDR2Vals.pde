...
void draw() {
  ...
  while (port.available() > 0) {
    ...
    if (myString != null) {
      ...
      if (vallist.length >= 2) {
        ...
        float yoffset = map(myVal, 10, 1023, 0, 1);
  ...
  tempo = map(myVal2, 10, 1023, 100, 300); // Adjust tempo with mouseX
  ...
  float clr1 = map(myVal, 10, 900, 0, 255);
  float clr2 = map(myVal2, 10, 900, 0, 255);
  fill(clr1, 0, 0,120);
